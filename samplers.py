...
class LDSampler(object):
    ...
    def run_online(self, num_updates, samples_per_update, batched_cts,
        ...
        for batch in batched_cts:
            ...
            new = [doc for ss in batch for doc in ss]
            batch=new
            ...
            print(f"{sampler_name} iteration {t}, docs so far {docs_so_far}, log pred {self.log_preds[t - 1]}, time {end - start}")
    ...
    def log_pred(self, test_cts, Adk_mean, Bkw_mean):
        ...
        phi_hat = self.phi
        log_probs = {(d, w): cntr[w] * np.log(np.dot(eta_hat[d, :], phi_hat[:, w]))
